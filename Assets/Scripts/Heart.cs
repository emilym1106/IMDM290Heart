...
public class Heart : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numSphere; i++)
        {
            ...
            spheres[i].transform.position = Vector3.Lerp(startPosition[i] * 2f, endPosition[i] * 0.7f, lerpFraction);
            // For now, start positions and end positions are fixed. But what if you change it over time?
            //startPosition[i] = startPosition[i] * 1.005f; 
            endPosition[i] = endPosition[i] * 1.0015f;
